...
A=pbf1*ptf1+ptf2*ptf2+2*bf1*tf1+2*bf2*tf2+2*hw*tw; % Area of Section
...
esi = tolerance;
    ...
    %---------------- Fs -------------%
     ...
     Fsi(z)=2*bf1*R2*tf1*fsi;
     Fstani(z)=2*bf1*R2*tf1*fstani;% tangent steel force
     ...
     Fsi(z)=2*tw*R3*hw*fsi;
     Fstani(z)=2*tw*R3*hw*fstani;% tangent steel force
     ...
     Fsi(z)=2*bf2*R4*tf2*fsi;
     Fstani(z)=2*bf2*R4*tf2*fstani;% tangent steel force  
...
if it < itermax% iteration control
        ...
        fprintf('   Applied axial load: %1.3f (kN) - Yield strength axial force capacity of the section: %1.3f (kN) - Ultimate strength axial force capacity of the section: %1.3f (kN) \n\n',Ptarget*.001,FYn*.001,FUn*.001)
end
else;esi=0;end
ES=[.1*ey .2*ey .4*ey .6*ey .8*ey ey .2*esh .4*esh .6*esh .8*esh esh .2*esu .4*esu .6*esu .8*esu esu];q=size(ES,2);
...
title(' Ploting of the Top steel strain and Curvature ','Color','b');grid on
figure(8)
...
title(' Ploting of the steel fiber strain and Heigh of section - Negative value: Tension - Positive value: Compression','Color','b');grid on
...
title(' Ploting of the steel fiber stress and Heigh of section - Negative value: Tension - Positive value: Compression','Color','b');grid on
legend(['\phi= ',num2str(Cur(2))],['\phi= ',num2str(Cur(roundn(.25*q,0)))],['\phi= ',num2str(Cur(roundn(.5*q,0)))],['\phi= ',num2str(Cur(roundn(.75*q,0)))],['\phi= ',num2str(Cur(roundn(q+1,0)))],'Location','NorthEastOutside');
figure(10)
...
title(['# STEEL SECTION MOMENT CURVATURE DIAGRAM   - AXIAL LOAD= 100 (kN) #'],'Color','b'); 
xlabel('CURVATURE (1/m)');ylabel('MOMENT (kN-m)')
legend('Analysis','SAP2000','Location','NorthEastOutside');grid on;
%% Output Data to .txt file
fid = fopen('FiberIIPlateSteelSectionMomentCurvatureAxialLoad-OutPut.txt','w');
fprintf(fid,'*************************************************************\n');
fprintf(fid,'*                 >> IN THE NAME OF ALLAH <<                *\n');
fprintf(fid,'*   Moment-Curvature Analysis of Double I steel sections    *\n');
fprintf(fid,'*   with Plate on Flange and Axial Load effect              *\n');
fprintf(fid,'*-----------------------------------------------------------*\n');
fprintf(fid,'*      This program is written by Salar Delavar Qashqai     *\n');  
