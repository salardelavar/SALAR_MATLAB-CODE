...
esi = tolerance; % initialize strain guess
    ...
    %---------------- Fs -------------%
     ...
     Fsi(z)=2*bf1*R1*tf1*fsi;
     Fstani(z)=2*bf1*R1*tf1*fstani;% tangent steel force
     ...
     Fsi(z)=2*tw*R2*hw*fsi;
     Fstani(z)=2*tw*R2*hw*fstani;% tangent steel force
     ...
     Fsi(z)=2*bf2*R3*tf2*fsi;
     Fstani(z)=2*bf2*R3*tf2*fstani;% tangent steel force
       end
    end
...
fid = fopen('FiberIISteelSectionMomentCurvatureAxialLoad-OutPut.txt','w');
fprintf(fid,'*************************************************************\n');
