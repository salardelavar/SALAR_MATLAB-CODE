...
esi = tolerance; % initialize strain guess
    ...
    %---------------- Fs -------------%
     ...
     Fsi(z)=bf1*R1*tf1*fsi;
     Fstani(z)=bf1*R1*tf1*fstani;% tangent steel force
     ...
     Fsi(z)=tw*R2*hw*fsi;
     Fstani(z)=tw*R2*hw*fstani;% tangent steel force
     ...
     Fsi(z)=bf2*R3*tf2*fsi;
     Fstani(z)=bf2*R3*tf2*fstani;% tangent steel force
       end
    end
...
Ie1=((tw*hw^3)/12)+2*(((bf1*tf1^3)/12)+(bf1*tf1)*(0.5*h-0.5*tf1)^2);
phiY1=ey/(.5*h);phiU1=esu/(.5*h);
My1=(fy*Ie1)/(.5*h); % Yeild Moment
Mu1=fy*[2*bf1*tf1*(.5*h-.5*tf1)+(tw*hw^2)/4];  % Ultimate Moment
...
figure(2)
IMAGE1=imread('FiberISteelSectionMomentCurvature.jpg');
image(IMAGE1);axis image;axis off;
%% Plot
figure(3)
...
fid = fopen('FiberISteelSectionMomentCurvatureAxialLoad-OutPut.txt','w');
fprintf(fid,'*************************************************************\n');
