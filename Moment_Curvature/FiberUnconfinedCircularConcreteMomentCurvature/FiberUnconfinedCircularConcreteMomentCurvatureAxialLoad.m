...
% Cirular section with Axial Load effect                    %
%-----------------------------------------------------------%
...
As=[490.8594 981.7188 981.7188 981.7188 0 490.8594]; % NOTE: As1 & As6 = 8fi25
%d:d1  d2  d3  d4  d5  d6 
d=[37.5 99.74 250 400.26 0 462.26];
%% Concrete Properties
...
x=.5*D;% initial guess of Neuteral axis
%%% monitor cpu time
starttime = cputime;
%% ------------------ Newton Method Procedure ------------------------%
An=size(As,2);R=(1/N);
for k=1:N;c(k)=(.5*R+R*(k-1))*D;Dd(k)=2*(((.5*D)^2-(.5*D-c(k))^2)^.5);end
if abs(Ptarget)>0
...
for u=1:An;
    ...
    for z=1:N;% in this step: concrete force for each fiber is calculated
     ...
     Cci(z)=Dd(z)*R*D*Ci;
     Cctani(z)=Dd(z)*R*D*Ctani;% tangent concrete force
    end                
    %----------------------------------%
       ...
       FsTOTAL_tan=sum(Fstani);CcTOTAL_tan=sum(Cctani);FssTOTAL_tan=sum(Fstansi);A_tani=CcTOTAL_tan+FsTOTAL_tan+FssTOTAL_tan;
        ...
        if it == itermax % stop the the analysis of this step please of Convergence
          fprintf('(-) trail iteration reached to Ultimate %1.0f - strain: %1.6f - evaluate residual: [%1.2f]\n',it,eci,residual)
...
for j=1:q;
    ...
    for z=1:N;% in this step: concrete force for each fiber is calculated
     ...
     if and(ec>0,ec<ec0)
      ...
      elseif ec<ect3
        C=0;
        Ctan=0;
        CD(z)=D-c(z);% Crack Depth
     end
     Cc(z)=Dd(z)*R*D*C;
     Cctan(z)=Dd(z)*R*D*Ctan;% tangent concrete force
    ...
    Pc1=x-.5*D;% Distance from applied force from Neuteral axis
...
fprintf('\nTotal time (s): %7.4f \n\n',totaltime)
%% Moment-Curvature of C50x50-10fi25
CurP=[0
    1.299*10^-3
    2.727*10^-3
    6.753*10^-3
...
figure(2)
IMAGE2=imread('FiberUnconfinedCircularConcreteMomentCurvature.jpg');
...
figure(9)
plot(SS(1,:),(D-c),SS(roundn(.25*q,0),:),(D-c),SS(roundn(.5*q,0),:),(D-c),SS(roundn(.75*q,0),:),(D-c),SS(roundn(q,0),:),(D-c),'g--','LineWidth',3);xlabel('Concrete strain (mm/mm)');ylabel('Height (mm)')
...
figure(10)
plot(CFS(1,:),(D-c),CFS(roundn(.25*q,0),:),(D-c),CFS(roundn(.5*q,0),:),(D-c),CFS(roundn(.75*q,0),:),(D-c),CFS(roundn(q,0),:),(D-c),'g--','LineWidth',3);xlabel('Concrete stress (Mpa)');ylabel('Height (mm)')
...
fid = fopen('FiberUnconfinedCircularConcreteMomentCurvatureAxialLoad-OutPut.txt','w');
fprintf(fid,'*************************************************************\n');
fprintf(fid,'*                  >> IN THE NAME OF GOD <<                 *\n');
fprintf(fid,'*  Moment-Curvature analysis of Unconfined concrete         *\n');
fprintf(fid,'* Cirular section with Axial Load effect                    *\n');
fprintf(fid,'*-----------------------------------------------------------*\n');
