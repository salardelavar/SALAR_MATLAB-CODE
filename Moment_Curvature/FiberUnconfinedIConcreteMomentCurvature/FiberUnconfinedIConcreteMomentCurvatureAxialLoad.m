...
Ptarget =+5000;% [N] Target axial load [+ : Compression]
%% Section Properties
...
As=[3500 2000 2000 2000 2000 3500]; 
%d:d1  d2  d3  d4  d5  d6 
...
for u=1:An;
    ...
    for z=1:N;% in this step: concrete force for each fiber is calculated
     ...
     Cci(z)=bf1*R1*tf1*Ci;
     Cctani(z)=bf1*R1*tf1*Ctani;% tangent concrete force
     ...
     Cci(z)=tw*R2*hw*Ci;
     Cctani(z)=tw*R2*hw*Ctani;% tangent concrete force
     ...
     Cci(z)=bf2*R3*tf2*Ci;
     Cctani(z)=bf2*R3*tf2*Ctani;% tangent concrete force
     end
    end
    %----------------------------------%
...
for j=1:q;
    ...
    %----------------------------------%
    FsTOTAL=sum(Fs);CcTOTAL=sum(Cc);FssTOTAL=sum(Fss);A=CcTOTAL+FsTOTAL+FssTOTAL-Ptarget;
    FsTOTAL_tan=sum(Fstan);CcTOTAL_tan=sum(Cctan);FssTOTAL_tan=sum(Fstans);A_tan=CcTOTAL_tan+FssTOTAL_tan+FsTOTAL_tan;
    dx = A_tan^-1 *(-A);
...
fid = fopen('FiberUnconfinedIConcreteMomentCurvatureAxialLoad-OutPut.txt','w');
fprintf(fid,'*************************************************************\n');
fprintf(fid,'*                  >> IN THE NAME OF GOD <<                 *\n');
fprintf(fid,'* Moment-Curvature Analysis of Unconfined I concrete section*\n');
