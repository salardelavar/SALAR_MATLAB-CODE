...
% Checking the analysis by Force - Large displacement       %
...
P5=.1; % [kN]
P6=-1; % [kN]
D5max=500; % [mm] Maximum displacement [DOF (5)]
...
% initial assemble global K matrix
        %  5    6
Kini = [G1*lanX1^2+G2*lanX2^2 G1*lanX1*lanY1+G2*lanX2*lanY2;
...
G2=EA2/L2;
        %  5    6
 K = [G1*lanX1^2+G2*lanX2^2 G1*lanX1*lanY1+G2*lanX2*lanY2;
      G1*lanX1*lanY1+G2*lanX2*lanY2 G1*lanY1^2+G2*lanY2^2];        
        f=K*u-F;
        %calculate du1 & du2
        du = Kini^-1 *(-f);
...
% Force and Dispalcement for each increment
    ...
    if abs(u(2)) >= D6max;disp('      ## Displacement at [DOF (6)] reached to Ultimate Displacement ##');break;end
end
D1=[U2N'];F1=[TBSN1'];
disp('+ Large Displacement Analysis [DOF(6)] +');
...
% Force and Dispalcement for each increment
    ...
    if abs(u(1)) >= D5max;disp('      ## Displacement at [DOF (5)] reached to Ultimate Displacement ##');break;end
end
D2=[U1L'];F2=[TBSL2'];
disp('+ Large Displacement Analysis [DOF(5)] +');
